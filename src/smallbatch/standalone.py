...
```python
from smallbatch_functions.{module} import classify, classify_batch, metadata
```

The package does not depend on Smallbatch. `metadata()` returns its contract and evidence.
"""
...
if request["items"]:
    function.classify(request["items"][0])
...
for item in request["items"]:
    started = time.perf_counter()
    outputs.append(function.classify(item))
